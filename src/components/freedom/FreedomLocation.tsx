...
export default function FreedomLocation() {
  const { data: allLocations, isLoading } = useGetAllLocationsQuery("all");
  const [editingLocation, setEditingLocation] = useState<any>(null);

  // Filter for freedom camps
  const locations = allLocations?.filter((loc: any) => 
    loc.category?.toLowerCase().includes("freedom") || 
    loc.type?.toLowerCase().includes("freedom")
  );
    ...
    <div className="bg-zinc-950 rounded-lg overflow-hidden border border-[#12271F] px-6">
      ...
      {locations?.map((location: any) => (
        ...
        >
          ...
          {/* Type */}
          <TypeButton type={location.category || "Freedom Camp"} bg="bg-[#10B981]" />
