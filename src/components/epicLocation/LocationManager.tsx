...
export default function LocationManager() {
  ...
  // Filter for epic spots
  const locations = allLocations?.filter((loc: any) => 
    loc.category?.toLowerCase().includes("epic") || 
    loc.type?.toLowerCase().includes("epic")
  );
    ...
    <div className="bg-zinc-950 rounded-lg overflow-hidden border border-[#12271F] px-6">
      ...
      {locations?.map((location: any) => (
        ...
        >
          ...
          {/* Type */}
          <TypeButton icon={<EpicIcon />} type={location.category || "Epic Spot"} />
