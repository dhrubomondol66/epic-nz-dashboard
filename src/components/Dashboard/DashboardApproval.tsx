...
import { useGetAllLocationsQuery, useApproveLocationMutation, useRejectLocationMutation } from '../../redux/features/locationApi';
...
export default function DashboardApproval() {
  ...
  const [approveLocation] = useApproveLocationMutation();
  const [rejectLocation] = useRejectLocationMutation();

  const onApprove = async (id: string) => {
    try {
      await approveLocation(id).unwrap();
    } catch (err) {
  ...
  const onReject = async (id: string) => {
    try {
      if (window.confirm('Are you sure you want to reject this location?')) {
        await rejectLocation(id).unwrap();
      }
    } catch (err) {
                  ...
                  >
                    <div className="grid grid-cols-12 gap-4 items-center">
                      ...
                      <div className="col-span-3 flex items-center justify-end gap-3">
                        <button onClick={() => onApprove(approval._id)} className="w-8 h-8 rounded-lg bg-[#3BB774]/10 hover:bg-[#3BB774] flex items-center justify-center transition-all cursor-pointer text-[#3BB774] hover:text-white" title="Approve">
                          <Check className="w-4 h-4" />
                        </button>
                        <button onClick={() => onReject(approval._id)} className="w-8 h-8 rounded-lg bg-red-500/10 hover:bg-red-500 flex items-center justify-center transition-all cursor-pointer text-red-500 hover:text-white" title="Reject">
                          <X className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => navigate(`/submission?id=${approval._id}`)}
